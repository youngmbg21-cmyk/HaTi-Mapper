  ...
  return {
    ...
    event(ev) {
      ...
      if (ev.type === 'content_block_delta') {
        ...
        /* A thinking block is signed, and the signature arrives as its own
           delta. Losing it makes the block invalid the moment it is handed
           back on the next step of the tool loop — which is a thing this
           assistant does on almost every question. */
        if (d.type === 'signature_delta') { b.signature = (b.signature || '') + d.signature; return null; }
