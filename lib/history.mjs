...
export function snapshot(scan) {
  ...
  return {
    ...
    // The estimated cost of a full day at the caps HaTi's code sets. A number,
    // so a line can be drawn through it — see W12 and the trends strip.
    dailyCostUsd: s.cost?.dailyMixedUsd ?? null,
  };
}
...
export function diff(prev, next) {
  const out = [];
  const add = (weight, kind, text, tag) => out.push(tag ? { weight, kind, text, tag } : { weight, kind, text });
  ...
  for (const g of nG) if (!pG.has(g)) add(2, 'gaps', `A new known gap was written down: “${g}”.`, 'gap-opened');
  for (const g of pG) if (!nG.has(g)) add(1, 'gaps', `A known gap was closed or removed from the documents: “${g}”.`, 'gap-closed');
...
export class History {
  constructor(dir) {
    ...
    /* How many times the Mapper has looked at HaTi at all, as opposed to how
       many times it found something. A look that finds nothing leaves no entry
       — that is the point of the log — which meant the service could not say
       how often it had looked, and the page said it anyway using the snapshot
       count. "Looked 2 times and found nothing" and "looked 200 times and
       found nothing" mean opposite things, so the number has to be real. */
    this.looks = 0;
    this.writable = true;
  ...
  _load() {
    ...
    try {
      ...
      /* Absent in logs written before looks were counted. Starting such a log
         at the number of entries it has is closer to the truth than zero, and
         is never higher than the real figure. */
      this.looks = Number.isFinite(data.looks) ? data.looks : this.entries.length;
      this._prune();
    } catch (e) {
  ...
  _save() {
    ...
    try {
      ...
      fs.writeFileSync(this.file, JSON.stringify({ v: 1, looks: this.looks, entries: this.entries }));
    } catch (e) {
  ...
  /* The snapshot the next scan will be compared against, or null on a fresh
     install. The watch rules need it before `record` moves it on. */
  latestSnapshot() {
  ...
  /* Record a scan. Returns the events this scan introduced (empty when
     nothing moved). `extra` is for events worked out elsewhere — a tripped
     watch rule — which belong in the log alongside the derived ones. */
  record(scan, extra) {
    const snap = snapshot(scan);
    const last = this.entries[this.entries.length - 1];
    const added = (extra || []).filter(e => e && e.text);

    /* Counted before anything is decided, because a look that finds nothing is
       still a look — and is exactly the one the rest of this method leaves no
       trace of. */
    this.looks += 1;
    ...
    const events = [...added, ...diff(last.snap, snap)];
    if (!events.length) {
  ...
  /* How the "Not finished" list has moved over a window. Counted from tagged
     events rather than by matching their wording, so a reworded sentence
     cannot silently break the tally. */
  gapMovement(days = 30) {
    const rounds = this.changes(days * 24);
    let opened = 0, closed = 0;
    for (const r of rounds) {
      for (const e of r.events) {
        if (e.tag === 'gap-opened') opened++;
        else if (e.tag === 'gap-closed') closed++;
      }
    }
    return { days, opened, closed, watchedSince: this.entries[0] ? this.entries[0].at : null };
  }
  ...
  status() {
    ...
    return {
      ...
      /* Every look ever taken, including the ones that found nothing. The
         difference between this and `snapshots` is how often HaTi was
         unchanged, which is the thing that makes "nothing has moved" either
         reassuring or meaningless. */
      looks: this.looks,
