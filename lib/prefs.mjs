...
const DEFAULTS = {
  ...
  /* The launch checklist's owner-answered items: { <id>: { done, at } }. The
     timestamp is not bookkeeping — several of these items go stale, and a tick
     with no date on it cannot be told from one taken in March. */
  launch: {},
...
export class Prefs {
  ...
  _load() {
    ...
    try {
      ...
      this.data = {
        ...
        launch: raw.launch && typeof raw.launch === 'object' ? raw.launch : {},
      };
    } catch (e) {
  ...
  /* One item on the launch checklist. `allowed` is the set of ids the
     checklist actually defines; an id outside it is refused rather than
     stored, because a tick nothing draws is a tick that counts for nothing and
     looks like progress in the file. Un-ticking deletes rather than storing
     `false`, so the file never accumulates a record of every box ever touched.

     The timestamp is stamped here rather than accepted from the browser: it is
     what several of these items are checked against for staleness, and a date
     the client could choose is a date the client could use to keep a stale
     tick alive forever. */
  setLaunch(id, done, allowed) {
    if (!allowed || !allowed.has(id)) return false;
    const next = { ...(this.data.launch || {}) };
    if (done) next[id] = { done: true, at: new Date().toISOString() };
    else delete next[id];
    this.data.launch = next;
    this._save();
    return true;
  }
