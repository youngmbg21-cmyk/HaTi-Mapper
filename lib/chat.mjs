 ...
 * 72-hour change log, and the live caps — so it cannot reach HaTi's contracts
 * even in principle.
 *
...
export const CHAT_TOOLS = [
  ...
  {
    name: 'get_changes',
    description: 'What has actually changed in the platform recently, from the Mapper\'s own 72-hour watch log. Use for "what changed", "what is new", "has anything moved since yesterday". This is different from `commits` — it is what the Mapper noticed, in plain terms, not the git log.',
    input_schema: {
      type: 'object',
      properties: { hours: { type: 'number', description: 'How far back to look, up to 72. Defaults to 72.' } },
    },
  },
  {
    name: 'search_map',
...
export function buildSystem({ scan, historyStatus, pulse }) {
  const s = scan || {};
  const counts = {
...
- For "what changed" or "what's new", use get_changes — that is the Mapper's own 72-hour watch log, in plain terms. The "commits" panel is the raw git history and is usually the less useful answer.
...
export function runTool(name, input, ctx) {
  ...
  switch (name) {
    ...
    case 'get_changes': {
      ...
      const hours = Math.min(Math.max(Number(a.hours) || 72, 1), 72);
