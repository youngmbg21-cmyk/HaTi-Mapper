...
import { SCREEN_COPY, FEATURE_COPY, HASH_COPY, FILE_COPY, GROUP_COPY, TABLE_COPY } from '../data/copy.js';
 ...
 *
 * The parameter list is walked past first, by matching its parentheses. Taking
 * the first `{` after the name instead looks right and is wrong the moment a
 * parameter is destructured — `function f(key, { text, kind })` hands back
 * " text, kind " as the body, and every fact the caller wanted to read out of
 * the real body comes back missing. That is how HaTi's playbook endpoint came
 * to be reported as costing nothing. */
function functionBody(src, name) {
  ...
  for (let i = paren; i < src.length; i++) {
    const ch = src[i];
    if (ch === '(') depth++;
    else if (ch === ')') { depth--; if (depth === 0) { afterParams = i + 1; break; } }
  }
...
function scanAi(files) {
  ...
  /* Every AI endpoint that costs money.
   *
   * The test is HaTi's OWN declaration: it tags every paid route with
   * `aiFeature('name')` in the middleware chain, and tags none of the free
   * ones. That tag is what its spend ledger and its budget guard key on, so a
   * route carrying it is a paid route by the product's own definition.
   *
   * This used to search the handler for a literal `anthropicMessages(` call
   * instead, and got two answers wrong in opposite directions on the real
   * HaTi. `/api/ai/playbook` calls Anthropic through a helper rather than
   * directly, so a genuinely paid endpoint was reported as costing nothing —
   * the money panel was under-counting. And `/api/ai/log`, which is an admin
   * read of the chat history and calls nothing, was reported as a paid
   * feature, because a route's body runs to the start of the next route and
   * had swallowed a neighbour that does call Anthropic. Reading the tag is
   * both more accurate and less clever.
   */
  const routes = parseRoutes(server);
  const billed = r => /aiFeature\(\s*'[^']+'\s*\)/.test(r.middleware);

  /* The tag is looked for on EVERY route, not only on the ones under
     /api/ai/. HaTi bills `/api/templates/upload` as an AI feature —
     converting an uploaded document into a template costs a deep call — and
     filtering by path prefix missed it entirely, so a paid endpoint appeared
     on no panel at all and the day's ceiling was short by its share. Where a
     route costs money is HaTi's to say, and it says it with the tag. */
  const aiRoutes = routes.filter(r => !r.unparsed && billed(r));

  /* Everything under the AI prefix that is NOT billed: configuration, the
     spend read-outs, the allowance admin. Reported separately so the reader
     can see they were looked at and cost nothing. */
  const nonBillingAiRoutes = routes
    .filter(r => !r.unparsed && r.path.startsWith('/api/ai/') && !billed(r))
    .map(r => ({ method: r.method.toUpperCase(), path: r.path, label: `${r.method.toUpperCase()} ${r.path}` }));

  if (!aiRoutes.length) {
    warnings.push('No route anywhere carries an aiFeature() tag — HaTi may have changed how it marks a paid endpoint, and the money panel will be empty until that is followed.');
  }

  /* The body of a named helper, so a handler that calls Anthropic one hop away
     can still have its tier and its answer ceiling read. */
  const helperBody = (name) => functionBody(server, name);
  ...
  const features = aiRoutes.map(r => {
    ...
    /* Where the Anthropic call actually is. Usually in the handler; sometimes
       one hop away in a named helper the handler calls, which is how
       /api/ai/playbook is written. Both are followed, and where neither turns
       one up the route says so rather than being silently priced at nothing. */
    /* What an Anthropic call looks like here. `anthropicMessages` is the
       ordinary one and `anthropicMessagesStream` the streaming one; both are
       real spend, and matching only the first left HaTi's streaming Copilot
       looking free. */
    const CALLS = /\banthropicMessages(?:Stream)?\(\s*key\s*,/;

    let where = r.body;
    let via = null;
    if (!CALLS.test(where)) {
      /* Every helper the handler calls is tried, and the one that actually
         carries the call wins — not merely the first whose body mentions it.
         Taking the first match found the middleware tag rather than the
         helper doing the work. */
      for (const call of r.body.matchAll(/\b(\w+)\s*\(/g)) {
        if (call[1] === r.path) continue;
        const body = helperBody(call[1]);
        if (body && CALLS.test(body)) { where = body; via = call[1]; break; }
      }
    }

    /* Tier: read from the anthropicMessages() call. It is usually a literal;
       where a route chooses per request the choice is resolved from the
       ternary, whether that ternary is written into the call itself
       (`anthropicMessages(key, compared ? 'deep' : 'fast', …)`) or into a
       variable assigned above it. Only a genuinely unresolvable one warns. */
    let tiers = [];
    const inline = where.match(/anthropicMessages(?:Stream)?\(\s*key\s*,\s*[^,;)]*?\?\s*'(\w+)'\s*:\s*'(\w+)'/);
    const call = where.match(/anthropicMessages(?:Stream)?\(\s*key\s*,\s*(?:'(\w+)'|(\w+))/);
    if (inline) {
      tiers = [inline[2], inline[1]];              // condition false first: the common path
    } else if (call && call[1]) {
      tiers = [call[1]];
    } else if (call && call[2]) {
      const v = call[2];
      const tern = where.match(new RegExp(`(?:const|let)\\s+${v}\\s*=\\s*[^;]*?\\?\\s*'(\\w+)'\\s*:\\s*'(\\w+)'`));
      if (tern) tiers = [tern[2], tern[1]];
      else warnings.push(`${r.path}: tier is the variable "${v}" and could not be resolved from the handler.`);
    } else {
      warnings.push(`${r.path}: it is tagged as a paid feature but no anthropicMessages() call was found in its handler or in a helper it calls — tier and model not derived.`);
    }

    /* The ceiling on one answer. It is the only number in the source that
       bounds what a single request can cost, so it is what the cost estimate
       is built from — and it is read wherever the call turned out to be. */
    const cap = where.match(/max_tokens\s*:\s*(\d+)/);
    ...
    return {
      ...
      /* Named when the Anthropic call is not in the handler itself, so the
         panel can say where the figures came from rather than appearing to
         read a route that does not obviously call anything. */
      callVia: via,
      limiter: limiterName,
  ...
  /* One row per FEATURE, not per route.
   *
   * HaTi meters spend by the aiFeature tag, and it tags more than one address
   * with the same name — the Copilot answers on /api/ai/chat and again on
   * /api/ai/chat/stream, both booked against "chat". Left as two rows they
   * are one feature counted twice, which would overstate the ceiling and put
   * the same thing in the spend split twice. Merged, the row carries every
   * address it answers on, the widest tier either uses, and the larger answer
   * ceiling — a ceiling should describe the worst case, not the first one
   * found. */
  const merged = [];
  const byFeature = new Map();
  for (const f of features) {
    const seen = byFeature.get(f.feature);
    if (!seen) {
      byFeature.set(f.feature, f);
      f.routes = [f.route];
      merged.push(f);
      continue;
    }
    seen.routes.push(f.route);
    seen.tiers = [...new Set([...seen.tiers, ...f.tiers])];
    seen.models = seen.tiers.map(t => tierDefaults[t] || NOT_DETECTED);
    if (f.maxTokens != null) seen.maxTokens = Math.max(seen.maxTokens || 0, f.maxTokens);
    if (seen.cap == null) seen.cap = f.cap;
  }

  const modelsInUse = [...new Set(merged.flatMap(f => f.models).filter(m => m && m !== NOT_DETECTED))];

  return { features: merged, caps, tierDefaults, modelsInUse, windowMinutes: windowMin, nonBillingAiRoutes, warnings };
}
...
function scanStorage(files) {
  ...
  for (let m; (m = re.exec(server));) {
    ...
    tables.push({ name: m[1], columns, line: lineOf(server, m.index), holds: TABLE_COPY[m[1]] || null });
  }
...
function scanDependencies(files) {
  ...
  const groupIds = new Set((DEPENDENCIES.groups || []).map(g => g.id));
  ...
  for (const s of DEPENDENCIES.subsystems) {
    ...
    /* The panel groups the tiles by this, so a subsystem with no group — or
       one naming a group that does not exist — would silently vanish from the
       board. It says so instead. */
    if (!s.group) warnings.push(`Subsystem "${s.title}" has no group in data/dependencies.js, so it cannot be placed on the board.`);
    else if (!groupIds.has(s.group)) warnings.push(`Subsystem "${s.title}" is in a group "${s.group}" that is not defined in data/dependencies.js.`);
  }
...
function scanPublic(files, hashRoutes) {
  const server = text(files, 'server/server.js');
  const warnings = [];
  /* No server file, no route list — but the shape has to be the same shape,
     because everything downstream reads it without asking whether the scan got
     that far. totalRoutes is null rather than 0 on purpose: 0 is a claim that
     HaTi has no addresses, and what actually happened is that the Mapper could
     not tell. Leaving the key off entirely, which is what this used to do, is
     the worst of the three — it printed "of undefined server routes" on the
     control tower, and disappeared without trace from the assistant's copy,
     since JSON.stringify drops an undefined and takes the fact with it. */
  if (!server) return { routes: [], hashes: [], totalRoutes: null, warnings: ['server/server.js not found.'] };
 ...
 * says so on screen.
 */
/* The menu's sections in menu order, named. An id with no name in the
   phrasebook keeps its raw id and is reported as a copy warning rather than
   being quietly prettified into something that might be wrong. */
function screenGroups(screens) {
  const out = [];
  const seen = new Set();
  for (const s of screens) {
    const id = s.group || 'other';
    if (seen.has(id)) continue;
    seen.add(id);
    out.push({
      id,
      label: GROUP_COPY[id] || id,
      named: !!GROUP_COPY[id],
      /* The Mapper's own group rather than one of HaTi's, and the one the
         reader most needs marked: everything in it works without a login. */
      open: id === 'public',
    });
  }
  return out;
 ...
 *
 * HaTi opens almost every module with a comment naming it — `// HaTi — the
 * redline engine: what changed, as storable ops.` — and the phone modules use
 * a ruled block saying the same thing. That is a convention in HaTi's own
 * source, so reading it is the same kind of act as every other parser here:
 * deriving a fact rather than inventing one. It sits at the BOTTOM of the
 * precedence list in describeFiles, under the dependency map, the screens and
 * the hand-written phrasebook, because a sentence somebody wrote for the owner
 * beats a sentence somebody wrote for the next programmer.
 *
 * Before this, a file nothing else could explain showed as "no plain-English
 * note yet" with no sentence at all. Against HaTi as it now stands that was 31
 * of 59 files, including the biggest one in the product.
 */
function ownWords(files, path) {
  const src = text(files, path);
  if (!src) return null;

  /* The opening comment only. Reading further would start quoting whatever
     comment happened to sit above the first function. */
  const head = src.slice(0, 4000);
  const block = head.startsWith('/*')
    ? (head.match(/^\/\*([\s\S]*?)\*\//) || [, ''])[1]
    : (head.match(/^(?:[ \t]*\/\/.*(?:\r?\n|$))+/) || [''])[0];
  ...
  const body = block
    ...
    // Rule lines drawn out of = or - carry no words.
    .filter(l => l && !/^[=\-*_]{3,}$/.test(l))
    .join(' ')
    // "HaTi — " prefixes most of them and says nothing the reader needs.
    .replace(/^HaTi\s*[—–-]\s*/i, '')
    .replace(/\s+/g, ' ')
    .trim();
  if (!body) return null;

  /* Sentence boundaries, and only real ones: a full stop counts only where
     whitespace and a capital follow it, so `components.js`, `.docx` and
     `DESIGN-contract-designer.md §3` do not cut a sentence in half.

     SPLIT, not match. Matching sentences with a lookahead silently drops
     whatever falls between two matches, which is how one file came to be
     described as "Js); the module also exports for node" — the sentence the
     boilerplate filter below was meant to catch had been torn in half before
     the filter ever saw it. A split cannot lose text. */
  const sentences = body.split(/(?<=[.!?])\s+(?=[A-Z(“"])/).map(s => s.trim()).filter(Boolean);

  /* Two things that are not a description of THIS file.

     Nearly every module carries the same note about globals being
     window-attached. It is true of all of them, so it distinguishes none of
     them, and taking it would describe four different files identically.

     And several open with a shouted label — "THE PHONE.", "WHAT THIS FILE
     IS." — before saying anything. A sentence with no lower-case letter in it
     anywhere is a heading, not a sentence. That rule keeps the ones that only
     START shouting: "THE PHONE — one contract." is a real description. */
  const boilerplate = /globals?\s+(?:are\s+)?window-attached|window-attached like every|see components\.js/i;
  const shoutedLabel = s => !/[a-z]/.test(s);

  const sentence = sentences.find(s =>
    s.length >= 12 && s.length <= 220 && !boilerplate.test(s) && !shoutedLabel(s));
  if (!sentence) return null;

  /* HaTi writes its own titles as "<name>: <what it is>" often enough to be
     worth splitting on, and shouts some of them. */
  const parts = sentence.match(/^([^:]{3,60}):\s*(.{12,})$/);
  const tidy = s => sentenceCase(s.replace(/\s*[.!?]+$/, '').toLowerCase().replace(/\bhati\b/g, 'HaTi'));
  if (parts) {
    return { name: tidy(parts[1]), does: sentenceCase(parts[2].replace(/\s*$/, '')), from: 'header' };
  }
  /* No title in the sentence, so the file's own name is the label and its own
     words are the description. The path is on the row either way. */
  const stem = (path.split('/').pop() || path).replace(/\.[a-z]+$/i, '').replace(/[-_]+/g, ' ');
  return { name: sentenceCase(stem), does: sentenceCase(sentence), from: 'header' };
}

function describeFiles({ screens, routes, serverPath, files, paths, biggestPath }) {
  const said = new Map();
  ...
  if (serverPath && routes != null) {
    /* This used to end "That is why it is the biggest file in the product",
       which was true when it was written and is not any more — a front-end
       view has since overtaken it. A sentence that states a ranking has to
       check the ranking, or the panel ends up contradicting the bar chart
       drawn directly underneath it. */
    const biggest = biggestPath === serverPath;
    said.set(serverPath, {
      name: 'The engine room',
      does: `Every request HaTi answers arrives here — all ${routes} of them, from signing in to saving a contract.` +
        (biggest ? ' That is why it is the biggest file in the product.' : ''),
      from: 'routes',
    });
  }
  ...
  /* Last: the file's own opening comment, for anything still unexplained. */
  if (files) {
    for (const p of paths || []) {
      if (said.has(p)) continue;
      const own = ownWords(files, p);
      if (own) said.set(p, own);
    }
  }
...
function scanHealth(p) {
  ...
  /* Whether each file could be explained, not merely listed. This is the exact
     decay the grip figure exists to catch — a panel filling up with "no
     plain-English note yet" while every other number still looks healthy — and
     it was the one panel not counted here. */
  for (const f of p.weight.files) fact(!!f.does);
...
export function buildScan({ files, commits, repo, ref, requestCount, tarballCommit }) {
  ...
  const tableNames = new Set(storage.tables.map(t => t.name));
  for (const key of Object.keys(TABLE_COPY)) {
    if (!tableNames.has(key)) copyWarnings.push(`data/copy.js describes a table "${key}" that no longer exists in HaTi.`);
  }
  for (const t of storage.tables) {
    if (!t.holds) copyWarnings.push(`The "${t.name}" table has no plain-English description in data/copy.js.`);
  }
  const groupIds = new Set(screens.screens.map(s => s.group || 'other'));
  for (const key of Object.keys(GROUP_COPY)) {
    if (!groupIds.has(key)) copyWarnings.push(`data/copy.js names a menu section "${key}" that no longer exists in HaTi's nav.`);
  }
  for (const id of groupIds) {
    if (!GROUP_COPY[id]) copyWarnings.push(`HaTi's nav has a section "${id}" with no name in data/copy.js — the Screens board shows its raw id.`);
  }
  ...
  const described = describeFiles({
    ...
    files,
    paths: weight.files.map(f => f.path),
    // The bar chart's own answer, so the sentence and the bars cannot disagree.
    biggestPath: weight.files[0]?.path || null,
  });
  ...
  /* A file the Mapper cannot say anything about is a hole in the panel, and
     until now it was a silent one: the row simply showed "no plain-English
     note yet" and the grip figure — whose whole job is to make that kind of
     decay visible — never counted it. */
  weight.undescribed = weight.files.filter(f => !f.does).map(f => f.path);
  for (const p of weight.undescribed) {
    weight.warnings.push(`${p} has no plain-English note: nothing in data/copy.js describes it, no screen resolves to it, and its own opening comment could not be read.`);
  }
  ...
  const payload = {
    ...
    screens: screens.screens,
    /* The menu's sections, in the order the menu lists them, each with the
       name written in data/copy.js. The ORDER is derived — it is the order the
       sections appear in HaTi's own markup — so the board on the Screens tab
       reads the way the product's own menu reads, rather than alphabetically
       or in whatever order a map literal happened to be typed. */
    screenGroups: screenGroups(screens.screens),
