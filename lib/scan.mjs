...
function toGap(bullet) {
  ...
  if (bold) {
    ...
    if (/:$/.test(lead) && rest) {
      ...
      return { title: sentenceCase(title), detail, severity: null, marker: lead.replace(/:$/, '') };
    }
    ...
    if (/^[,;—-]/.test(rest)) {
      ...
      return { title, detail, severity: null, marker: null };
    }

    return { title: lead.replace(/[.:]$/, ''), detail: rest.replace(/^\.\s*/, '') || null, severity: null, marker: null };
  }
