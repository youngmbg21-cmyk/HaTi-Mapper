...
export function buildDigest({ rounds, points, scan, period = 'midnight', now = new Date() }) {
  ...
  /* Commits arrive newest-first, the way GitHub lists them. A morning report
     reads better the other way round: number one is the first thing the
     session did, and the last number is where it left off. So they are turned
     into story order here and numbered once, on the server, so the page and
  ...
  if (!events.length && !commits.length) {
    ...
    /* Not "commits": the reader is not a developer, and not "changes"
       either, because that word is spoken for by what the Mapper observed. */
    if (commits.length) bits.push(plural(commits.length, 'code update', 'code updates'));
...
export function digestText(digest, opts = {}) {
  ...
  if (digest.commits.length) {
    lines.push(`The ${digest.commits.length} code update${digest.commits.length === 1 ? '' : 's'}, in the order they happened`);
    for (const c of digest.commits) lines.push(`  ${c.n}. ${c.subject} (${c.sha})`);
    lines.push('');
  }

  if (!digest.scanCount) {
    lines.push(digest.commits.length
      ? 'The code updates above come straight from GitHub\'s record. The Mapper itself has not taken a look yet ' +
        `${digest.label} — anything it notices of its own will appear after the next scan.`
      : `The Mapper has not taken a look yet ${digest.label}.`);
    lines.push('');
  }
