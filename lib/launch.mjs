  ...
  },
  {
    ...
    check(ctx) {
      ...
      if (!rule.on) {
        return {
          state: 'open',
          detail: 'The AI-requests tripwire is off. It can be armed from here, or under the gear if you want to choose the number first.',
          /* Some of what the Mapper measures, the Mapper can also fix. Sending
             the owner to hunt for a toggle under a gear, three screens from
             the sentence explaining why it matters, is how a checklist item
             stays open for a month. Only ever offered for something the Mapper
             genuinely owns — nothing here can reach into HaTi. */
          action: { kind: 'watch-rule', rule: 'aiRequests', threshold: rule.threshold || 150, label: 'Arm it at ' + (rule.threshold || 150) + ' a day' },
        };
      }
